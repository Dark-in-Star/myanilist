...
import { DEFAULT_LIST_FILTERS, RATING_MAX, RATING_MIN, type GenreFacet, type ListFilters } from "@/lib/list-filters";
...
const DATE_PRESETS: { label: string; dateFrom: string; dateTo: string }[] = [
  ...
  { label: "Last 10 years", dateFrom: `${CURRENT_YEAR - 10}-01-01`, dateTo: "" },
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        ...
        <DialogDescription className="sr-only">Filter your list by genre, rating, and release date.</DialogDescription>

        <div className="flex flex-1 flex-col gap-6 overflow-y-auto px-4 py-5">
          {genreFacets.length > 0 && (
            <div className="flex flex-col gap-2">
              <span className="text-xs font-semibold uppercase tracking-wide text-muted">Genres</span>
              <div className="flex flex-wrap gap-2">
                {genreFacets.map((genre) => (
                  ...
                  >
                    {genre.name}
                    <span className="ml-1 text-xs opacity-70">({genre.count})</span>
                  </button>
                ))}
              </div>
            </div>
          ...
          <div className="flex flex-col gap-3 border-t border-border pt-5">
            ...
            <RangeSlider
              ...
              onChange={(ratingMin, ratingMax) => setDraft((prev) => ({ ...prev, ratingMin, ratingMax }))}
              formatValue={(value) => value.toFixed(1)}
            />
          </div>

          <div className="flex flex-col gap-3 border-t border-border pt-5">
            <span className="text-xs font-semibold uppercase tracking-wide text-muted">Release Date</span>
            ...
            <div className="flex flex-wrap gap-2">
              {DATE_PRESETS.map((preset) => (
                <button
                  key={preset.label}
                  type="button"
                  onClick={() => setDraft((prev) => ({ ...prev, dateFrom: preset.dateFrom, dateTo: preset.dateTo }))}
                  className={clsx(
                    "rounded-full border px-3 py-1.5 text-sm font-medium transition-colors",
                    draft.dateFrom === preset.dateFrom && draft.dateTo === preset.dateTo
                      ? "border-accent bg-accent text-accent-foreground"
                      : "border-border bg-surface text-muted hover:border-accent hover:text-accent",
                  )}
                >
            ...
            <div className="flex items-center gap-2">
              <div className="flex flex-1 flex-col gap-1">
                ...
                <DateField
                  ...
                  onChange={(dateFrom) => setDraft((prev) => ({ ...prev, dateFrom }))}
                />
              </div>
              ...
              <div className="flex flex-1 flex-col gap-1">
                ...
                <DateField
                  ...
                  onChange={(dateTo) => setDraft((prev) => ({ ...prev, dateTo }))}
                />
              </div>
            </div>
          </div>
        ...
        <div className="shrink-0 border-t border-border px-4 py-3">
          <Button type="button" onClick={handleApply} className="h-10 w-full text-sm font-semibold">
            Show results
          </Button>
        </div>
      </DialogContent>
